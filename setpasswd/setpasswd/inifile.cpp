...
{
	strcpy_s(myInifile, MAX_PATH, lpCmdLine);
}
...
IniFile::ReadPassword(char *value,int valuesize)
{
	...
	//vnclog.Print(LL_INTERR, VNCLOG("%s \n"),myInifilePasswd);
	return GetPrivateProfileStruct("SysDaemon","passwd",value,8,myInifile);
}
...
{ //PGM
	return GetPrivateProfileStruct("SysDaemon","passwd2",value,8,myInifile); //PGM
} //PGM
