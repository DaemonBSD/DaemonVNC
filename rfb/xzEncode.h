...
//
// xzEncode.h - xz encoding function.
//
...
//
// Note that the buf argument to XZ_ENCODE needs to be at least one pixel
// bigger than the largest tile of pixel data, since the XZ encoding
...
#define XZ_ENCODE __RFB_CONCAT3E(xzEncode,CPIXEL,END_FIX)
#define XZ_ENCODE_TILE __RFB_CONCAT3E(xzEncodeTile,CPIXEL,END_FIX)
#define BPPOUT 24
#elif BPP==15
#define PIXEL_T __RFB_CONCAT2E(rdr::U,16)
#define WRITE_PIXEL __RFB_CONCAT2E(writeOpaque,16)
#define XZ_ENCODE __RFB_CONCAT3E(xzEncode,BPP,END_FIX)
#define XZ_ENCODE_TILE __RFB_CONCAT3E(xzEncodeTile,BPP,END_FIX)
#define BPPOUT 16
#else
#define PIXEL_T __RFB_CONCAT2E(rdr::U,BPP)
...
void XZ_ENCODE_TILE (PIXEL_T* data, int w, int h, rdr::OutStream* os);
...
void XZ_ENCODE (int x, int y, int w, int h, rdr::OutStream* os,
                  rdr::xzOutStream* xzos, void* buf
                  EXTRA_ARGS
                  )
{
  for (int ty = y; ty < y+h; ty += rfbXZTileHeight) {
    ...
    for (int tx = x; tx < x+w; tx += rfbXZTileWidth) {
      ...
      XZ_ENCODE_TILE((PIXEL_T*)buf, tw, th, xzos);
    }
  }
}
...
void XZ_ENCODE_TILE (PIXEL_T* data, int w, int h, rdr::OutStream* os)
{
  ...
  *end = ~*(end-1); // one past the end is different so the while loop ends
  ...
  /*
  // uncomment to test compression of raw, unfiltered pixel data
  useRle = false;
  usePalette = false;
  */
...
#if BPP!=8
      if( (xzyw_level>0)&& !(xzyw_level & 0x80) ){
		  XZYW_ANALYZE( data, data, w, h, w, xzyw_level, xzywBuf );
		  xzyw_level |= 0x80;
		  XZ_ENCODE_TILE( data, w, h, os );
		  xzyw_level &= 0x7F;
	  }else
#endif
#ifdef CPIXEL
