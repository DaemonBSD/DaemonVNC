...
BOOL CUGP(char * userin,char *password,char *machine,char *groupin,int locdom)
{
	...
	VARIANT var;
	////////////FIND SERVER NEEDED FOR WINDOWS NT4
	DWORD dwRet;
	...
	wcscat_s(szPath, MAX_PATH, pdci->DomainControllerName+2);
	wcscat_s(szPath, MAX_PATH, L"/rootDSE");
	wprintf(szPath);
	wprintf(L"\n");
					   ...
					   IADs **ppUser) //Return a pointer to the user
{
	...
	wcscpy_s(pszSearchFilter, MAX_PATH, L"(&(objectCategory=person)(objectClass=user)(samAccountName=");
