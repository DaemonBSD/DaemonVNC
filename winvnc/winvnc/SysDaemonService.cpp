...
void WINAPI SysDaemonService::service_main(DWORD argc, LPTSTR* argv) {
	...
	if (strcmp(argv[0], service_name) == NULL) {
		strcpy_s(configfilename, "sysdaemon.ini");
	}
	else
	{
	...
	if (!file.good()) {
		...
		strcat_s(inifile, "\\");
		strcat_s(inifile, "sysdaemon.ini");
	}
...
void SysDaemonService::set_service_description()
{
	...
	_sprintf_s_s(tempName,  sizeof tempName, "SYSTEM\\CurrentControlSet\\Services\\%s", service_name);
...
BOOL SysDaemonService::CreateServiceSafeBootKey()
{
	...
	LONG lSuccess;
	char szKey[1024];
	_sprintf_s_s(szKey, 1024, "SYSTEM\\CurrentControlSet\\Control\\SafeBoot\\%s\\%s", "Network", service_name);
